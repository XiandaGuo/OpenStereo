...
__global__ void sga_down_forward (const int n, const float *filters, const int channel, const int height,
...
//  int wsize=radius+1;
  ...
  int fbase = index / (channel*width) * step * wsize + index % width;
...
__global__ void sga_down_data_backward (const int n, const float *filters, float *top_diff,
			const float *idx, const int channel, const int height, const int width,
...
for(int k=0; k<channel; k++){
  int base = index / step * step * depth * channel + index % step + k*step*depth;	//up->down
...
__global__ void sga_up_forward (const int n, const float *filters, const int channel, const int height,
		const int width, const int depth, const int wsize,
		float *top_data){
  ...
  int base = index / width * step * depth + index % width;	//up->down
  int fbase = index / (width*channel) * step * wsize + index % width;
...
__global__ void sga_right_forward (const int n, const float *filters, const int channel, const int height,
		   const int width, const int depth, const int wsize,
		   float *top_data){
  ...
  int fbase = index / (height*channel) * step * wsize + (index % height) * width;
...
__global__ void sga_right_weight_backward (const int n, const float *bottom_data,
			   const float *top_data, const float *temp_diff,
			   const float *idx, const int channel, const int height,
			   const int width, const int depth, const int wsize,
			   float *filters_diff){
...
__global__ void sga_left_forward (const int n, const float *filters, const int channel, const int height,
		  const int width, const int depth, const int wsize,
		  float *top_data){
  ...
  int base = index / height * step * depth + (index % height) * width;	//up->down
  int fbase = index / (channel*height) * step * wsize + (index % height) * width;
...
//      cudaStream_t stream = at::cuda::getCurrentCUDAStream(); 
  ...
  sga_down_forward <<< threads, CUDA_NUM_THREADS >>> (n, g0, channel, height, width,
...
//      cudaMemset( top_mask, 0, sizeof(float)*N);
  ...
  cudaMemcpy (top_temp, bottom_data, sizeof (float) * N,
	      cudaMemcpyDeviceToDevice);
  sga_up_forward <<< threads, CUDA_NUM_THREADS >>> (n, g1, channel, height, width,
						    depth, wsize, top_temp);
  ...
  cudaMemcpy (top_temp, bottom_data, sizeof (float) * N,
	      cudaMemcpyDeviceToDevice);
  sga_right_forward <<< threads, CUDA_NUM_THREADS >>> (n, g2, channel, height, width,
						       depth, wsize,
						       top_temp);
  Max <<< (N + CUDA_NUM_THREADS - 1) / CUDA_NUM_THREADS,
  ...
  cudaMemcpy (top_temp, bottom_data, sizeof (float) * N,
	      cudaMemcpyDeviceToDevice);
  sga_left_forward <<< threads, CUDA_NUM_THREADS >>> (n, g3, channel, height, width,
						      depth, wsize, top_temp);
  Max <<< (N + CUDA_NUM_THREADS - 1) / CUDA_NUM_THREADS,
...
//              (n,g3,height,width,depth,wsize,top_temp);
  ...
  sga_left_data_backward <<< (n + CUDA_NUM_THREADS - 1) / CUDA_NUM_THREADS,
    CUDA_NUM_THREADS >>> (n, g3, top_grad, idx, channel, height, width, depth, wsize,
			  grad_input);
  n = num * width * height;
  sga_left_weight_backward <<< (n + CUDA_NUM_THREADS - 1) / CUDA_NUM_THREADS,
    CUDA_NUM_THREADS >>> (n, bottom_data, top_temp, top_grad, idx, channel, height,
...
//backward for down             
  ...
  sga_down_forward <<< (n + CUDA_NUM_THREADS - 1) / CUDA_NUM_THREADS,
    CUDA_NUM_THREADS >>> (n, g0, channel, height, width, depth, wsize, top_temp);
  ...
  sga_down_data_backward <<< (n + CUDA_NUM_THREADS - 1) / CUDA_NUM_THREADS,
    CUDA_NUM_THREADS >>> (n, g0, top_grad, idx, channel, height, width, depth, wsize,
			  grad_input);
  n = num *  width * height;
...
// backward for up              
  ...
  sga_up_forward <<< (n + CUDA_NUM_THREADS - 1) / CUDA_NUM_THREADS,
    CUDA_NUM_THREADS >>> (n, g1, channel, height, width, depth, wsize, top_temp);
  ...
  sga_up_data_backward <<< (n + CUDA_NUM_THREADS - 1) / CUDA_NUM_THREADS,
    CUDA_NUM_THREADS >>> (n, g1, top_grad, idx, channel, height, width, depth, wsize,
			  grad_input);
  n = num * width * height;
...
//backward for right            
  ...
  sga_right_forward <<< (n + CUDA_NUM_THREADS - 1) / CUDA_NUM_THREADS,
    CUDA_NUM_THREADS >>> (n, g2, channel, height, width, depth, wsize, top_temp);
  ...
  sga_right_data_backward <<< (n + CUDA_NUM_THREADS - 1) / CUDA_NUM_THREADS,
    CUDA_NUM_THREADS >>> (n, g2, top_grad, idx, channel, height, width, depth, wsize,
			  grad_input);
  n = num * width * height;
