...
EfficientFormer_width = {
    ...
    'S0': [32, 48, 96, 176],  # 75.0 75.7 [24, 32, 96, 160]
}
...
class Attention4D(torch.nn.Module):
    ...
    def forward(self, x):  # x (B,N,C)
        ...
        out = x.transpose(2, 3).reshape(B, self.dh, v_local.size()[2], v_local.size()[3]) + v_local
...
class Attention4DDownsample(torch.nn.Module):
    ...
    def forward(self, x):  # x (B,N,C)
        ...
        out = x.reshape(B, self.dh, v_local.size()[2], v_local.size()[3]) + v_local
