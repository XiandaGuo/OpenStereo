...
class GetCostVolume(nn.Module):
    ...
    def forward(self, x, y, disp_range_samples, ndisp):
        ...
        cost[:, x.size()[1]:, :, :, :] = F.grid_sample(y, grid.view(bs, ndisp * height, width, 2), mode='bilinear',
                                                       padding_mode='zeros', align_corners=True).view(bs, channels,
                                                                                                      ndisp, height,
                                                                                                      width)
...
class PSMNet(nn.Module):
    def __init__(self, cfgs):
        ...
        print(
            "***********ndisps:{}  disp_interval_pixel:{} grad_method:{} ns:{}  ns_size:{} cr_base_chs:{} ************".format(
        ...
        self.cost_agg = nn.ModuleList([CostAggregation(in_channels=cr_feats_in_chs[i], base_channels=self.cr_base_chs[i])
        ...
        for stage_idx in range(self.num_stage):
            ...
            if self.training:
                ...
                pred3 = self.cost_agg[stage_idx](cost,
                                                 ...
                                                 disp_range_samples=disp_range_samples)
                pred = pred3
                outputs_stage = {"pred": pred}
                outputs["stage{}".format(stage_idx + 1)] = outputs_stage

        outputs['disp_pred'] = outputs[f"stage{len(self.ndisps)}"]['pred']
        return outputs
    ...
    def get_loss(self, model_pred, input_data):
        ...
        total_loss = torch.tensor(0.0, dtype=disp_gt.dtype, device=disp_gt.device, requires_grad=False)
        dlossw = [0.5, 1.0]

        for (stage_inputs, stage_key) in [(model_pred[k], k) for k in model_pred.keys() if "stage" in k]:
            disp0, disp1, disp2, disp3 = stage_inputs["pred0"], stage_inputs["pred1"], stage_inputs["pred2"], stage_inputs["pred3"]
            loss = 0.5 * F.smooth_l1_loss(disp0[mask], disp_gt[mask], reduction='mean') + \
                   0.5 * F.smooth_l1_loss(disp1[mask], disp_gt[mask], reduction='mean') + \
                   0.7 * F.smooth_l1_loss(disp2[mask], disp_gt[mask], reduction='mean') + \
                   1.0 * F.smooth_l1_loss(disp3[mask], disp_gt[mask], reduction='mean')
