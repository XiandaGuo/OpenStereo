...
OPTIMIZATION:
    ...
    AMP: false                # float32 if false
    # AMP_DTYPE: bf16   # used only when AMP: true;  bf16 if AMP=true and bf16;  float16 if AMP=true and no AMP_DTYP.

    # Iter-based training takes priority when MAX_ITER > 0, see trainer_template.py, L38–43:
    ...
    MAX_ITER: 31250   # effective
    NUM_EPOCHS: 64    # will be ignored when MAX_ITER > 0 (set to -1 to avoid confusion)
